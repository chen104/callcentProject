...
public class PowerLinkRunable1 implements Runnable {
    String data;
    ...
    public PowerLinkRunable1(String data, Map<String, Object> conversationContext, Handler handler, String intentAction) {
        this.data = data;
        this.list = list;
        ...
        try {
            ...
            if (jstat != null&&jstat.getAsBoolean()) {
              ...
              JsonObject  pwoerImte =  data.getAsJsonObject();
                String link="";
                if(RcmsConstant.IntentGetPowerPart.equals(intentAction)){
                    JsonElement JLING = pwoerImte.get(PowerItemConstant.partLink);
                    if(JLING!=null&&!"".equals(JLING.getAsString())){
                        link=JLING.getAsString();
                        conversationContext.put(RcmsConstant.power_part_key,"找到");

                        Message m =new Message();
                        m.msgType = Constant.chat_item_src_img;
                        m.message = link;
                        addMessage(m);
                    }
                    JsonElement jimg = pwoerImte.get(PowerItemConstant.partiamge);
                    if(jimg!=null&&!"".equals(jimg.getAsString())){
                        imgParent=imgParent+"part"+File.separator+jimg.getAsString();
                    }

                }else{
                    JsonElement JLING = pwoerImte.get(PowerItemConstant.localLink);
                    if(JLING!=null&&!"".equals(JLING.getAsString())){
                        link=JLING.getAsString();
                        conversationContext.put(RcmsConstant.power_local_key,"找到");
                        Message m =new Message();
                    ...
                    JsonElement jimg = pwoerImte.get(PowerItemConstant.localiamge);
                    if(jimg!=null&&!"".equals(jimg.getAsString())){
...
interface  PowerItemConstant{
    public static String machinModel="machinModel";
    public static String localLink="localLink";
    public static String partLink="partLink";
    public static String  partiamge="partiamge";
    public static String  localiamge="localiamge";
}
